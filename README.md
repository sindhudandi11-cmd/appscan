...
1. run on `http://localhost:3000` by default, or update `APP_FRONTEND_URL`
2. call the backend at `http://localhost:8080`
3. use the metadata endpoints to populate form selects
4. submit form data to `POST /api/applications`
5. fetch saved applications from `GET /api/applications`
...
4. Load dropdown options from `/api/meta/*`
5. Submit an application
6. View AI summary, score, and eligibility returned by the backend
